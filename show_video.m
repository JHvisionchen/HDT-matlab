...
%   http://www.isr.uc.pt/~henriques/
    ...
    struct('color',[163,73,164]/255,'lineStyle','-.')};%purple
		...
		if isempty(rect_h),  %create it for the first time
			rect_h = rectangle('Position',[0,0,1,1], 'LineWidth',3,'EdgeColor','g', 'Parent',axes_h);
        end
        if ~isempty(refbox)
            if isempty(ref_h)
                delete(rect_h);
               for i=1:length(refbox)-1
                  ref_h(i)= rectangle('Position',[0,0,1,1], 'LineWidth',2,'EdgeColor',brushstyle{i}.color, 'Parent',axes_h);
               end
